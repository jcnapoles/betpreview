...
public class CompetitionResource {
    ...
    public ResponseEntity<List<Competition>> getAllCompetitions(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Competitions");
        Page<Competition> page;
        if (eagerload) {
            page = competitionService.findAllWithEagerRelationships(pageable);
        } else {
            page = competitionService.findAll(pageable);
        }
