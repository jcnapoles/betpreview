...
export class CompetitionComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.competitionService
        ...
        .subscribe(
          (res: HttpResponse<ICompetition[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInCompetitions(): void {
    this.eventSubscriber = this.eventManager.subscribe('competitionListModification', () => this.loadPage());
  }
